...
struct
  ...
  fun diddle_edge change {from=(g:graph, i),to=(g':graph, j)} = 
      let val _ = check(g,g')
          ...
          val out_edge_exist = (List.exists (fn x => x = j) si) orelse (List.exists (fn x => x = j) pi)
          val _ = if out_edge_exist then () else A.update(g,i,NODE{succ=change(j,si),pred=pi})
          val NODE{succ=sj,pred=pj} = A.sub(g,j)
          (* val in_edge_exist = List.exists (fn x => x = j) pi *)
          val _ = if out_edge_exist then () else A.update(g,j,NODE{succ=sj,pred=change(i,pj)})
       in ()
      end
