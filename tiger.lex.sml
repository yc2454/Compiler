...
STRING | COMMENT | ESCPSTRING | INITIAL
...
fun yyAction44 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm; (Tokens.INT(get_int yytext, yypos, yypos+size yytext))
      end
fun yyAction45 (strm, lastMatch : yymatch) = (yystrm := strm;
      (cmtDepth := !cmtDepth+1; YYBEGIN COMMENT; continue()))
fun yyAction46 (strm, lastMatch : yymatch) = (yystrm := strm;
      (cmtDepth := !cmtDepth+1; continue()))
fun yyAction47 (strm, lastMatch : yymatch) = (yystrm := strm;
      (cmtDepth := !cmtDepth-1; if (!cmtDepth = 0) then (YYBEGIN INITIAL; continue()) else continue()))
fun yyAction48 (strm, lastMatch : yymatch) = (yystrm := strm;
      (lineNum := !lineNum+1; linePos := yypos :: !linePos; continue()))
fun yyAction49 (strm, lastMatch : yymatch) = (yystrm := strm; (continue()))
fun yyAction50 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN STRING; builder := ""; strpos := yypos; unclose := true; continue()))
fun yyAction51 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN INITIAL; unclose := false; Tokens.STRING(!builder, !strpos, yypos+1)))
fun yyAction52 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm;
        (builder := !builder ^ valOf(String.fromString yytext); continue())
      end
fun yyAction53 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN ESCPSTRING; continue()))
...
fun yyAction57 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (ErrorMsg.error yypos ("Illegal escape string " ^ yytext); YYBEGIN STRING; continue())
      end
...
fun yyAction59 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm; (builder := !builder ^ yytext; continue())
      end
       ...
       of NONE => yyAction39(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #">"
              ...
              then if inp = #"="
                  then yyQ106(strm', yyMATCH(strm, yyAction39, yyNO_MATCH))
            ...
            if inp = #"\\"
              ...
              then if inp = #"\""
                  ...
                  then if inp = #"\n"
                      ...
                      else yyQ7(strm', yyMATCH(strm, yyAction53, yyNO_MATCH))
                else if inp = #"0"
...
in
  (case (!(yyss))
   of STRING => yyQ0(!(yystrm), yyNO_MATCH)
    ...
    | ESCPSTRING => yyQ2(!(yystrm), yyNO_MATCH)
